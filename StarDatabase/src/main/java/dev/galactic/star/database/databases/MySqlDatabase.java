...
public class MySqlDatabase extends StarDatabase {
    ...
    public void createTables(Class<?>... tables) throws SQLException {
        ...
        if (tables.length == 0) {
            ...
            for (Class<?> table : tables) {
                String mainSb = "CREATE TABLE IF NOT EXISTS " + table.getName() + "(";
                ...
                mainSb.append(!isEmpty ? "PRIMARY KEY(" + table.getPrimaryKey() + "));" : ");");*/
                PreparedStatement ps = this.getConnection().prepareStatement(mainSb);
                ps.execute();
                ps.close();
            }
            if (this.isDebug()) {
    ...
    private void generateCreationString(Class<?> table) throws InvalidClassException {
        if (this.isDebug()) {
            System.out.println("Generating creation String...");
        }
        ...
        if (this.isDebug()) {
            System.out.println("Generated creation String!");
        }
    }
}
